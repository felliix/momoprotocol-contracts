...
contract Lottery is LotteryOwnable, Initializable {
    ...
    // Q Why is this not a struct?
    // TODO Either make a struct or limit the uint256 array to 4
    ...
    // Q not sure why an init is being used rather than the constructor?
    // TODO move lottoNFT, momo, admins and min + max into constructor
    constructor() public {}
    ...
    function initialize(
        ...
        // Q why is this hardcoded?
        // TODO either hardcode at delaration or pass in.
        allocation = [50, 12, 8];
        initOwner(_owner);
    }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        momo.safeTransfer(address(msg.sender), _amount);
        ...
        returns (uint64[keyLengthForEachBuy] memory)
    {
        ...
        // Q Why is this not an internal function that takes in:
        // uint8 _number, uint8 _position (0-10)
        // Q Why are the generations for each position different?
        // Changing it to repeated code would help with the size of this contract

        result[0] =
            tempNumber[0] *
            256 * 256 * 256 * 256 * 256 * 256 + 1 * 256 * 256 * 256 * 256 * 256 +
            tempNumber[1] *
            256 * 256 * 256 * 256 + 2 * 256 * 256 * 256 +
            tempNumber[2] *
            256 * 256 + 3 * 256 +
            tempNumber[3];
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external inDrawingPhase {
        ...
        momo.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external inDrawingPhase {
        ...
        momo.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward > 0) {
            momo.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if (totalReward > 0) {
            momo.safeTransfer(address(msg.sender), totalReward);
        }
        emit MultiClaim(msg.sender, totalReward);
    }

    // ------------------------------------------------------------------------
    // ADMIN PROTECTED EXTERNAL FUNCTIONS
